...
function dwscripts_getBalancedSelection(dom, balanceInward)
{
  ...
  dom.expandSelectionToIncludeActiveContentScript();
...
extUtils.setEditableToParent = extUtils_setEditableToParent;
...
//--------------------------------------------------------------------
// FUNCTION:
//   extUtils.setEditableToParent
//
// DESCRIPTION:
//   
//   If the node represents an editable region tag, set it to its parent node.
//   (So, for example, an editable region inside a <head> will match against <head>.
//
// ARGUMENTS:
//   node - a node from a params array
//
// RETURNS:
//   nothing
//--------------------------------------------------------------------
...
function extUtils_getSelTableRowOffsets()
{
  ...
  if (node.nodeType==Node.ELEMENT_NODE && (node.tagName == "TD" || node.tagName == "TH" || node.tagName == "TR") )
  {
   if (offsets.length == 2  &&
