  ...
  if (tag.indexOf("PreparedStatement") != -1)
  {
    ...
    if (tag.search(re) != -1)
    {
      if (RegExp.$1 == RegExp.$2)
      {
        var name = RegExp.$1;
        var SQL = RegExp.$3;
        var setObjectsText = RegExp.$4;
        SQL = SQL.replace(/\s*\)\s*;/, "");
        var parameters = null;

        var parameterArray = this.processSQL(SQL, setObjectsText);
...
function translator_processSQL(SQL, setObjectsText)
{
  ...
  if (setObjectsText && setObjectsText.search) {
    var pattern = "setObject\\(\\d+\\s*,\\s*(\\w+__)(\\w+)\\);"
    var re = new RegExp(pattern, "i");
    var index = setObjectsText.search(re);
    while (index != -1)
    {
      parameters = parameters + "\n<MM_PARAMETER NAME=\"" + RegExp.$1 + RegExp.$2 + "\">";
      SQL = SQL.replace(/\?/, RegExp.$2);
      setObjectsText = setObjectsText.replace(re, "");
      index = setObjectsText.search(re);
    }
  }
